...
class DeleteCapability(val userId: String, val deviceId: String, val capabilityType: String): ConfigOperation() {
                    ...
                    if (deviceId == curDeviceId) {
                        ...
                        for ((capIdx, capability) in capabilitiesNode.withIndex()) {
                            ...
                            if(capabilityNodeType == capabilityType) {
                                ...
                                capabilitiesNode.remove(capIdx)
                                break
                            }
                        }

                        if (!foundCapability) {
                            error("Capability type \"$capabilityType\" not found!")
                        }
                    }
                }
